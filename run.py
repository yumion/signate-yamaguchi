...
def parse_args():
    ...
    parser.add_argument('--scene-id', help = 'scene_id', default = '00')
...
def main():
    ...
    # load the input data
    test_video_path = os.path.join(data_dir, 'scene_{}.mp4'.format(args.scene_id))
    test_annotation_path = os.path.join(data_dir, 'scene_{}.json'.format(args.scene_id))
    ...
    print('\nPrediction for scene id: {}'.format(args.scene_id))
    with open(test_annotation_path, encoding='utf-8') as f:
        annotations = json.load(f)
    flags = make_flag(args.scene_id, annotations)
    prediction = ScoringService.predict(test_video_path)
    if not isinstance(prediction, list):
        print('Invalid data type. Must be list.')
        return None
    scene_ids = [args.scene_id]*len(prediction)
